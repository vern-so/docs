...
You'll need to [set up a webhook endpoint](/migration-api/webhooks) first.
...
Vern handles circular links between sheets cleanly. The payload schemas for both formats are in the [Webhooks](/migration-api/webhooks#payload-format) reference.

## Next

- [Webhooks](/migration-api/webhooks) — set up the endpoint that catches exports.
