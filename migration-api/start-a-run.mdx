...
<ParamField body="kind" type="string" required>
  ...
  - **`generate`** — author the mapping recipe from the uploaded files and stop at
    a preview. Requires at least one [uploaded file](/migration-api/upload-files).
...
The recipe and preview state are copied **server-side** between phases — you never
send recipe, preview, or build IDs. `execute` runs the recipe behind the current
